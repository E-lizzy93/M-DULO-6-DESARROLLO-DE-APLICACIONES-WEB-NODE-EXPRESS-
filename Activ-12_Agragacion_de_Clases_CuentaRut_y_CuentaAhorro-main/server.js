...
const CuentaRut    = require('./models/CuentaRut');
const CuentaAhorro = require('./models/CuentaAhorro');

const PORT         = 3000;
const ARCHIVO_JSON = path.join(__dirname, 'clientes.json');
const SALDO_MINIMO = 1000; // saldo mínimo en cuenta RUT para abrir ahorro
...
function generarIdCliente(clientes) {
  return clientes.length > 0 ? Math.max(...clientes.map(c => c.id)) + 1 : 1;
}

// El número de cuenta RUT es el RUT sin dígito verificador
// Ej: RUT 12345678-9 → número cuenta RUT: 12345678
function generarNumeroCuentaRut(rut) {
  return rut.split('-')[0].trim();
}

// Generar número de cuenta Ahorro autoincremental
function generarNumeroCuentaAhorro(clientes) {
  const nums = clientes.flatMap(c => c.cuentasAhorro)
...
const server = http.createServer((req, res) => {
  ...
  // ── GET /api/clientes/buscar?rut=12345678-9 → buscar cliente por RUT ──
  ...
  // ── POST /api/clientes → crear cliente nuevo con cuenta RUT ───────────
    ...
    leerBody(req, (err, datos) => {
      ...
      leerClientes((err, clientes) => {
        ...
        const cuentaRut       = new CuentaRut(numeroCuentaRut, datos.saldoRut);
        ...
        // Si quiere también cuenta ahorro
        if (datos.abrirAhorro) {
          if (datos.saldoRut < SALDO_MINIMO) {
            return responderJSON(res, 400, {
              error: `Saldo insuficiente. Se requieren mínimo $${SALDO_MINIMO.toLocaleString()} en cuenta RUT para abrir cuenta ahorro`
            });
          }
          const saldoAhorro  = datos.saldoAhorro || 0;
          const numeroAhorro = generarNumeroCuentaAhorro(clientes);
          // Descontar el saldo de la cuenta RUT
          nuevoCliente.cuentaRut.saldo -= saldoAhorro;
          nuevoCliente.cuentasAhorro.push(new CuentaAhorro(numeroAhorro, saldoAhorro));
        }
    ...
    leerBody(req, (err, datos) => {
      ...
      leerClientes((err, clientes) => {
        ...
        if (cliente.cuentaRut.saldo < SALDO_MINIMO) {
          return responderJSON(res, 400, {
            error: `Saldo insuficiente. Se requieren mínimo $${SALDO_MINIMO.toLocaleString()} en cuenta RUT`
          });
        }

        const saldoAhorro  = datos.saldo || 0;
        const numeroAhorro = generarNumeroCuentaAhorro(clientes);
        const nuevaCuenta  = new CuentaAhorro(numeroAhorro, saldoAhorro);

        // Descontar el saldo de la cuenta RUT al abrir la cuenta ahorro
  ...
  // ── DELETE /api/clientes/:id → cerrar cuenta principal (eliminar cliente)
    ...
    leerClientes((err, clientes) => {
      ...
      // Calcular saldo total para que el ejecutivo informe al cliente
  ...
  // ── DELETE /api/clientes/:id/ahorro/:numAhorro → cerrar cuenta ahorro ──
    ...
    leerClientes((err, clientes) => {
      ...
      // Eliminar solo esa cuenta ahorro
